...
public class MongoRepositoryImpl extends AbstractRepository implements MongoRepository
{
	...
	protected final MongoRepositoryCollection mongoRepositoryCollection;
	...
	{
		return find(QueryBuilder.start("_id").in(ids));
	}
	...
	protected List<Entity> find(QueryBuilder qb)
	{
		return collect(dbCollection.find(qb.get()));
	}

	protected List<Entity> collect(DBCursor cursor)
	{
		try
		{
